...
for (const flavor of args.flavors)
	if (!available('flavors').includes(flavor))
		fail(`unknown flavor "${flavor}" (have: ${available('flavors').join(', ')})`)
...
for (const flavor of args.flavors) copyLayer(join(templateRoot, 'flavors', flavor))
...
console.log(`\nnext steps:`)
console.log(`  1. ${isBun ? 'bun' : 'pnpm'} install   # also installs git hooks`)
